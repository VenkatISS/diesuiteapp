...
function addRow(){
	...
	if(ele.length<4){
		...
		var i = newRow.insertCell(7);
		var j = newRow.insertCell(8);
		var k = newRow.insertCell(9);
		var l = newRow.insertCell(10);
		...
		i.innerHTML = "<td><select name='sid' class='form-control input_field select_dropdown' id='sid' style='width:100px;'>"+staffdatahtml+"</select></td>";
...
/*		var cust = formobj.cust_id.selectedIndex;*/
		...
		if(elements.length==1) {
			...
			ems = validateEntries(sidate,cust,item,up,upd,qty,prec,psvc,pt,samt,sgsta,cgsta,productId);
		}else if (elements.length>1){
			for(var i=0; i<elements.length; i++){				
...
function validateEntries(sidate,cust,item,up,upd,qty,prec,psvc,pt,samt,sgsta,cgsta,productId){
