...
//		var h = newRow.insertCell(7);
		...
		f.innerHTML = "<td><input type=text class='form-control input_field eadd' name='siamt' id='siamt' size='6' readonly='readonly' style='background-color:#F3F3F3;' placeholder='Sale Amount'></td>";
		g.innerHTML = "<td><img src='images/delete.png' onclick='doRowDeleteInTranxs(this,siamt,si_amt,data_table_body)'></td>"+
							"<td>"+
...
function saveData(obj){
	...
	if(document.getElementById("epid") != null){		
		...
		if(elements.length==1) {
			...
			ems = validateEntries(sidate,cust,item,up,upd,qty,pt,samt,sgsta,cgsta);
		}else if (elements.length>1){
			for(var i=0; i<elements.length; i++){
...
function validateEntries(sidate,cust,item,up,upd,qty,pt,samt,sgsta,cgsta){
