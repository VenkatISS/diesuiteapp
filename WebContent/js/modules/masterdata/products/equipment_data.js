...
function addRow() {
	...
	if(ele.length < 4){
		...
		a.innerHTML = "<td valign='top' height='4' align='center'><select name='epid' id='epid' onchange='checkForDeletedProducts(this)' class='form-control input_field select_dropdown ic sadd'>"+ctdatahtml+"</select></td>";
...
function checkForDeletedProducts(eqpobj) {
	...
	if(ele.length==1){
		var prod = formobj.epid.value.trim();
		var sprod = document.getElementById("epid").options;
...
//				formobj.ed.value = "";
		...
		for(var e=0;e<ele.length;e++){
			var prod = formobj.epid[e].value.trim();
			var sprod = formobj.epid[e].options;
...
function saveData(obj){
	...
	if(document.getElementById("epid") != null){
		...
		if(elements.length==1) {
			var eitem = formobj.epid.selectedIndex;
			...
			for(var i=0; i<elements.length; i++){
				var eitem = formobj.epid[i].selectedIndex;
