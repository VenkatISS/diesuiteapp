...
*/
  ...
  /*---------------------------------------------------------------------------------UJWALA  Conditions-----------------------------------------------------------------------------------------*/
	if((document.getElementById("ctypeul").checked == true) || (document.getElementById("ctypeuc").checked == true)){  //ujwala if start
		...
		else if(eqdp != 2 && (ctype == 3 || ctype == 4))
			str = str + "You can only have  DEPOSIT REQUIRED No For Equipment in Ujwala.  <br>";
		else 
			document.getElementsByClassName("crd")[0].selectedIndex = 2;
		...
		if(!(OnlyOneReg.length >=1))
			...
			var redp = document.getElementsByClassName("crd")[1].selectedIndex;
			if(redp != 2)
				str = str + "You can only have  DEPOSIT REQUIRED No For Regulator in Ujwala. <br>";
			else 
				document.getElementsByClassName("crd")[1].selectedIndex = 2;	
		}
