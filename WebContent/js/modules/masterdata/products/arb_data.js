...
for(var f=page_data.length-1; f>=0; f--){
	if(page_data[f].deleted == 0) {
		...
		tblRow.innerHTML = "<td>" + spd +  "</td>" + "<td>" + page_data[f].prod_brand + "</td>" + "<td>" + page_data[f].prod_name + "</td>" + 
			"<td>" + page_data[f].csteh_no + "<td>" + eus[page_data[f].units] + "</td>" + "<td>" + page_data[f].gstp +  "</td>" +   
			"</td>" + "<td>" + page_data[f].prod_mrp +  "</td>" + "<td>" + page_data[f].opening_stock +  "</td>" + 
...
function addRow() {
	...
	if(ele.length < 4) {
		...
		a.innerHTML = "<td><select name='pid' id='pid' onchange='checkForDeletedItems()' class='form-control input_field select_dropdown ic sadd'>"+ctdatahtml+"</select></td>";
		b.innerHTML = "<td><input type=text name='pbrand' id='pbrand' maxlength='20' onchange='checkForDeletedItems()' class='form-control input_field eadd brand' placeholder='PRODUCT BRAND'></td>";
		c.innerHTML = "<td><input type=text name='pname' id='pname' maxlength='20' onchange='checkForDeletedItems()' class='form-control input_field eadd pname' placeholder='PRODUCT NAME'></td>";
		d.innerHTML = "<td><input type=text name='cstehv' id='cstehv' maxlength='8' onchange='checkForDeletedItems()' class='form-control input_field eadd' placeholder='HSN CODE'></td>";
...
function checkForDeletedItems() {
	...
	if(ele.length==1){
		...
		for(var d=0;d<page_data.length;d++){
			if(page_data[d].deleted != 0) {
				if(((page_data[d].csteh_no).trim() == formobj.cstehv.value.trim()) && (page_data[d].prod_code == parseInt(prod)) 
						&& ((page_data[d].prod_brand).trim() == formobj.pbrand.value.trim()) && ((page_data[d].prod_name).trim() == formobj.pname.value.trim())) {

					// FOR ARB PRODUCT
					var item = page_data[d].prod_code;
		...
		for(var e=0;e<ele.length;e++){
			...
			for(var d=0;d<page_data.length;d++){
				if(page_data[d].deleted != 0) {	
					if(((page_data[d].csteh_no).trim() == formobj.cstehv[e].value.trim()) && (page_data[d].prod_code == parseInt(prod)) 
							&& ((page_data[d].prod_brand).trim() == formobj.pbrand[e].value.trim()) && ((page_data[d].prod_name).trim() == formobj.pname[e].value.trim())) {
...
function saveData(obj){
	...
	if(document.getElementById("pid") != null) {
		...
		if(elements.length==1) {
			...
			ems = validateEntries(eitem,epb,epn,eusitem,egst,ecsteh,esd,ee,ed);
			if(!(ems.length>0)) {
...
function fetchARBProductDetails(types, pc,pb,pn) {
...
function validateProduct2(text) {
    if (page_data.length != 0) {
    	for (var i = 0; i < page_data.length; i++) {
    		if(page_data[i].deleted ==0){
    			var spd = fetchARBProductDetails(page_data,page_data[i].prod_code,page_data[i].prod_brand,page_data[i].prod_name);
...
function validateProduct() {
	...
	if (eletype.length == 1) {
		...
		flag = validateProduct2(text+"-"+btext+"-"+ntext);
		return flag;
		...
		for (i = 0; i < eletype.length - 1; i++) {
			...
			flag = validateProduct2(e3+"-"+b3+"-"+n3);
			if (flag) {
				for (j = 0; j < eletype.length - 1; j++) {
					...
					flag = validateProduct2(e4+"-"+b4+"-"+n4);
					var k = 0;
					if (flag) {
						for (k; k < h && k < eletype.length - 1; k++) {
							...
							flag = validateProduct2(e5+"-"+b5+"-"+n5);
...
function validateEntries(item,pbrand,pname,units,gstp,cstehno,mrp, os, ed){
	var formobj = document.getElementById('page_add_form');
	var errmsg = "";
	...
	if (((item > 0) && (pbrand.length > 0) && (pname.length > 0)) && (!validateProduct()))
		errmsg = errmsg + "Please Select Another PRODUCT CATEGORY Which Was not Selected.<br>";
