...
//		rowpdhtml = rowpdhtml + "<tr valign='top'><td><a href='javascript:showInvoice("+page_data[z].id+","+page_data[z].credit_note_type+")'>"+page_data[z].sid+"</td>";
         if((page_data[z].credit_note_type != 1) && (page_data[z].deleted == 0))
		...
		rowpdhtml = rowpdhtml + "<td>"+page_data[z].credit_amount+"</td>";
...
/*		res=checkRefNoAndProduct(formobj,cvtype,crefno,parsedOmcVal);
		if(res==false)
			return;*/
		...
		ems = validateEntries(nidate,cvtype,cvoid,item,egst,tamt,igst,cgst,sgst,camt);
	...
	var bc;
	var tflag = 0;
	if(formobj.crtype.selectedIndex == 2) {
		...
		if(bank_data.length>0) {
			for(var y=0; y<bank_data.length; y++) {
				if(bank_data[y].id == bid){
					bc = bank_data[y].bank_code;
					bname = bc+" - "+ bank_data[y].bank_acc_no;
...
function validateEntries(nidate,cvtype,cvoid,item,egst,tamt,igst,cgst,sgst,camt) {
	...
	if(!(nidate.length>0))
		errmsg = errmsg+"Please enter CREDIT NOTE DATE<br>";
	else if (vd != "false")
		errmsg = errmsg +vd+"<br>";
	else if(validateDayEndAdd(nidate,dedate)){
        errmsg = "CREDIT NOTE DATE should be after DayEndDate<br>";
        return errmsg;
	}else if(validateEffectiveDateForCVO(nidate,effdate)){
        errmsg = "CREDIT NOTE DATE should be after Effective Date that you have entered While First Time Login<br>";
        return errmsg;
	}else if(chkd != "true")
		errmsg = errmsg+chkd+"<br>";
	else if (vfd != "false")
		errmsg = errmsg +"CREDIT NOTE "+vfd+"<br>";
/*	else {
...
function deleteItem(id,invdate){
	var flag=validateDayEndForDelete(invdate,dedate);
	if(flag){
	 $("#myDialogText").text("Are You Sure You Want To Delete?");
	 var formobj = document.getElementById('m_data_form');
	 confirmDialogue(formobj,5533,id);
	}else {
...
function setVC(){
	...
	if(si==0) {
		document.getElementById("cSpan").innerHTML = "<select id='cvo_id' name='cvo_id' class='form-control input_field select_dropdown sadd' onchange='selectSaleType()'><OPTION VALUE='0'>SELECT</OPTION></select>";
	} else if (si==1) {
		document.getElementById("cSpan").innerHTML = "<select id='cvo_id' name='cvo_id' class='form-control input_field select_dropdown sadd' onchange='selectSaleType()'>"+vendordatahtml+"</select>";
		
	} else if (si==2) {
		document.getElementById("cSpan").innerHTML = "<select id='cvo_id' name='cvo_id' class='form-control input_field select_dropdown sadd' onchange='selectSaleType()'>"+custdatahtml+"</select>";
	}
    	...
    	if(formobj.egst.selectedIndex > 0){
    		if(formobj.tamt.value.trim() != ""){
    			...
    			if(cvtype>0) {
    				...
    				document.getElementById("dialog-1").innerHTML = "please select CREDIT NOTE TYPE";
    				alertdialogue();
    				//alert("please select CREDIT NOTE TYPE");
    				return;
    			}

    			var selectedcvo = formobj.cvo_id.value;
    			var parsedcvoVal = parseInt(selectedcvo);
    			var cinv_date = formobj.ndate.value.trim();
    			var crefno = formobj.cinvno.value.trim();
    			var ptypev = formobj.stype.value;
    			
    			res=checkRefNoAndProduct(formobj,cvtype,crefno,cinv_date,parsedcvoVal);
    			if(res==false)
    				return;	
    			...
    			var ems = validateCalcValues(pid,cvtype,tamtv,gstpsi,ptypecheck,ptypev,nbid,nreason);
    			if(ems.length>0) {
    				document.getElementById("dialog-1").innerHTML = ems;
    				alertdialogue();
    				//alert(ems);
...
function validateCalcValues(pid,cvtype,tamtv,gstpsi,ptypecheck,ptypev,nbid,nreason) {
	var errmsg = "";
	var formobj = document.getElementById('data_form');
	...
	//}
	if(!(nreason>0))
		errmsg = errmsg+"Please select credit note REASON <br>";
...
function checkRefNoAndProduct(formobj,cvtype,crefno,cinv_date,parsedOmcValue) {
		...
		if(crefno.length >0) {
			if(cvtype==1) {   //receive
				...
				if((cprval=="") && (arbprval=="") && (puRvalue=="")) {
					...
					typeflag = "cp";
					res=checkProductInPurInv(formobj,pi_data,cprval,typeflag,parsedOmcValue,cinv_date);
					return res;
				}else if(puRvalue != "") {
					typeflag = "cpR";
					res=checkProductInPurInv(formobj,purchase_return,puRvalue,typeflag,parsedOmcValue,cinv_date);
					return res;
				}				
				else if(arbprval !="") {
					typeflag = "arbp";
					res=checkProductInPurInv(formobj,arb_pi_data,arbprval,typeflag,parsedOmcValue,cinv_date);
					return res;
				}else{ return true; }
				...
				if((dsval=="") && (csval=="") && (arbsval=="") && (salRval=="")) {
					...
					typeflag = "ds";
					res=checkProductInSaleInv(formobj,drs_data,dsval,typeflag,parsedOmcValue,cinv_date);
					return res;
				}else if(csval != "") {
					typeflag = "cs";
					res=checkProductInSaleInv(formobj,crs_data,csval,typeflag,parsedOmcValue,cinv_date);
					return res;
				}else if(arbsval != "") {
					typeflag = "arbs";
					res=checkProductInSaleInv(formobj,arbs_data,arbsval,typeflag,parsedOmcValue,cinv_date);
					return res;
				}
				else if(salRval != "") {
					typeflag = "sr";
					res=checkProductInSaleInv(formobj,sr_data,salRval,typeflag,parsedOmcValue,cinv_date);
					return res;
				}else {return true;}
			}	
		}else {
...
function checkProductInPurInv(formobj,pur_data,prval,typeflag,parsedOmcValue,cnInvDate) {
	var flag="NO";
	var pide = formobj.epid;
	var iprodar = new Array();	
	var omcId,invdel,invDate;
	var cnDate = (new Date(cnInvDate)).getTime();
	...
	if(flag=="YES") {
		if (pide.selectedIndex > 0) {
			if(omcId != parsedOmcValue){
				...
				alertdialogue();
				return false;
			}else if(cnDate < invDate) {
				document.getElementById("dialog-1").innerHTML = "Please SELECT Valid CREDIT NOTE DATE ";
				alertdialogue();
				return false;
			}else {
...
function checkProductInSaleInv(formobj,Sale_data,sval,typeflag,custVal,cnInvDate) {
	var flag="NO";
	var pide = formobj.epid;
	var iprodar = new Array();	
	var custId,invDate;
...
/*			for (var j=0;j<Sale_data[i].detailsList.length;j++) {
	...
	if(flag=="YES") {
		if (pide.selectedIndex > 0) {
//			for (var z=0;z<Sale_data.length;z++) {
			if(custId != custVal) {
...
function popitup(inv_id,si_date) { 
	var w=window.open("PopupControlServlet?actionId=990&sitype=10&sid="+inv_id +"&si_date="+si_date,null,"height=400,width=1024,status=yes,toolbar=no,menubar=no,location=no");
	w.print();
	return false; 
}